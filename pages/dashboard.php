...
// $stmt = $koneksi->prepare("SELECT * FROM tim WHERE ketua_id = ? OR EXISTS (
//   SELECT 1 FROM anggota_tim WHERE tim_id = tim.id AND user_id = ?
// )");
// $stmt->execute([$user_id, $user_id]);
// $userTim = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        <div class="content-section" data-category="notifikasi" style="display: none">
          <div class="dashboard-sidebar">
            ...
            <?php
              ...
              $stmt = $koneksi->prepare("SELECT * FROM notifikasi WHERE user_id = ? AND status_baca = 'belum_dibaca' ORDER BY created_at DESC");
              $stmt->execute([$ketua_id]);
              $notifs = $stmt->fetchAll(PDO::FETCH_ASSOC);

              foreach ($notifs as $notif) {
                  echo "<div class='notif-item'>";
                  echo htmlspecialchars($notif['pesan']);
                  echo "<a href='proses_respon_gabung.php?tim_id={$notif['terkait_tim_id']}&user_id={$notif['dari_user_id']}'>Kelola</a>";
                  echo "</div>";
              }
              ?>
        </div>
