document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("pengumuman-closeBtn")
  ...
  document.querySelectorAll(".table-content").forEach((row) => {
    row.addEventListener("click", function () {
      ...
      document.querySelector("#adminReplyForm input[name='parent_id']").value =
        topicId;
      ...
      fetch(`prosesForumAdmin.php?action=get_discussion&topic_id=${topicId}`)
        ...
        .then((data) => {
          if (data.success) {
            ...
            html += `
                        <div class="main-post" data-message-id="${
                          main.forum_id
                        }">
                            <div class="post-header">${
                              main.fullname
                            } <span class="delete-message-btn" data-id="${
              main.forum_id
            }">Hapus</span></div>
                            <div class="post-meta">${new Date(
                              main.waktu_postingan
            ...
            data.replies.forEach((reply) => {
              html += `
                            <div class="reply-post" data-message-id="${
                              reply.forum_id
                            }">
                                <div class="post-header">${
                                  reply.fullname
  ...
  document.querySelectorAll(".delete-discussion-btn").forEach((btn) => {
    btn.addEventListener("click", function (e) {
      ...
      if (
        confirm(
          "Yakin ingin menghapus seluruh diskusi ini (termasuk semua balasannya)?"
        )
      ) {
  ...
  detailContent.addEventListener("click", function (e) {
    if (e.target.classList.contains("delete-message-btn")) {
      ...
      if (
        confirm(
          "Yakin ingin menghapus pesan ini? Jika ini pesan utama, seluruh diskusi akan terhapus."
        )
      ) {
        const formData = new FormData();
